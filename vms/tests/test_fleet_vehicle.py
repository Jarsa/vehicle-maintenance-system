...
class TestFleetVehicle(TransactionCase):
    def setUp(self):
        ...
        self.address_home = self.env.ref("base.res_partner_address_3")
        self.supervisor.address_home_id = self.address_home.id
        self.vehicle_id.supervisor_id = self.supervisor.id
    ...
    def test_compute_distance_averange(self):
        self.vehicle_id._compute_distance_averange()
        self.assertEqual(self.vehicle_id.distance, 1000)

    def test_cron_vehicle_maintenance(self):
        self.vehicle_id._compute_distance_averange()
        self.vehicle_id.cron_vehicle_maintenance()
        ...
        self.vehicle_id.cron_vehicle_maintenance()
        mail = self.env["mail.message"].search([("res_id", "=", order.id)])
