...
class TestVmsOrder(TransactionCase):
    def setUp(self):
        ...
        self.service_product = self.env.ref("vms.product_product_vms_04")
    ...
    def order_with_reports(self):
        context = {"active_model": "vms.report", "active_ids": [self.report.id]}
        wizard = (
    ...
    def test_compute_end_date(self):
        ...
        end_date = fields.Datetime.to_string(
            fields.Datetime.from_string(order.start_date)
            + timedelta(hours=self.task.duration)
        )
    ...
    def test_compute_end_date_real(self):
        ...
        end_date_real = fields.Datetime.to_string(
            fields.Datetime.from_string(order.start_date_real)
            + timedelta(hours=order.order_line_ids.real_duration)
        )
    ...
    def test_action_open(self):
        ...
        with self.assertRaisesRegexp(
            ValidationError,
            "Unit not available for maintenance because it has more " "open order.",
        ):
            order_one.action_open()
            order_two = self.create_order("preventive")
            order_two.action_open()
        with self.assertRaisesRegexp(
            ValidationError, "The order must have at least one task."
        ):
