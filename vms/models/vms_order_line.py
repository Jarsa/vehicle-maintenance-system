...
class VmsOrderLine(models.Model):
    ...
    purchase_state = fields.Boolean(
        string="Purchase Order State Done", compute="_compute_purchase_state"
    )
            ...
            if rec.external:
                if not rec.purchase_state:
                    raise ValidationError(
                        _("Verify that purchase order are in done state to continue")
                    )
        ...
        items.append(
            (
                ...
                0,
                {
                    ...
                    "taxes_id": [
                        (6, 0, [x.id for x in (self.product_id.supplier_taxes_id)])
                    ],
