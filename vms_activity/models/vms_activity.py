...
class VmsActivity(models.Model):
    ...
    task_id = fields.Many2one("vms.task", string="Task", readonly=True)
    ...
    def _compute_total_hours(self):
        for rec in self:
            sum_time = 0.0
            for activity in rec.activity_time_ids.filtered(lambda r: r.state == "end"):
        ...
        for rec in self:
            ...
            if rec.state in ["draft", "end", "cancel"]:
                raise ValidationError(
                    _("The activity must be in Pending, Process or Pause")
                )
    ...
    def end_activity_time(self, rec):
        ...
        if self.state == "process":
            act_in_process = self.env["vms.activity.time"].search(
                [("activity_id", "=", rec.id), ("state", "=", "process")], limit=1
            )
            if act_in_process:
