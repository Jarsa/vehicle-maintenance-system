...
class TestVmsActivity(TransactionCase):
    def setUp(self):
        super().setUp()
        self.operating_unit = self.env.ref("operating_unit.b2b_operating_unit")
    ...
    def test_start_resume_activity_time(self):
        ...
        with self.assertRaisesRegexp(
            ValidationError, "The activity must be in Pending, Process or Pause"
        ):
            activity.action_cancel()
            activity.start_resume_activity_time()
        with self.assertRaisesRegexp(
            ValidationError, "The order line task must be open."
        ):
    ...
    def test_compute_total_hours(self):
        ...
        for act in activity.activity_time_ids:
            act.end_date = fields.Datetime.from_string(act.end_date) + timedelta(
                hours=0.5
            )
